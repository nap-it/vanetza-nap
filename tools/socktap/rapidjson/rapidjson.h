...
class VanetzaJSONException : public std::exception {
public:
    VanetzaJSONException(const std::string& cause) : cause(cause) {}
    ...
    const std::vector<std::string>& getContextList() const {
        return contextList;
    }
...
#define RAPIDJSON_ASSERT(x) if (!(x)) throw VanetzaJSONException("Unexpected JSON Encoding Error!")
#define RAPIDJSON_EXCEPTION(x,y) if (!(x)) throw VanetzaJSONException(y)
