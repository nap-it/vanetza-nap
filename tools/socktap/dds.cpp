...
Dds::Dds(PubSub* pubsub_, config_t config) {
    ...
    // Eagerly create the participant so the interface whitelist is applied before
    // any publisher/subscriber triggers lazy creation without it.
    const std::string& blocked = config.dds_blocked_interfaces.empty()
        ? config.interface
        : config.dds_blocked_interfaces;
    get_participant(this->participantName, this->domain_id, blocked);
}
