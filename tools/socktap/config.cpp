...
message_config_t read_message_config(INIReader reader, string env_prefix, string ini_section) {
    message_config_t res = {
        ...
        getenv((env_prefix + "_MQTT_TIME_ENABLED").c_str()) == NULL ? reader.GetBoolean(ini_section, "mqtt_time_enabled", true)
                                                                    : (strcmp(getenv((env_prefix + "_MQTT_TIME_ENABLED").c_str()), "true") == 0),
        getenv((env_prefix + "_MQTT_TEST_ENABLED").c_str()) == NULL ? reader.GetBoolean(ini_section, "mqtt_test_enabled", false)
                                                                    : (strcmp(getenv((env_prefix + "_MQTT_TEST_ENABLED").c_str()), "true") == 0),
...
void read_config(config_t* config_s, string path) {
    ...
    config_s->ignore_own_messages =
        getenv("VANETZA_IGNORE_OWN_MESSAGES") == NULL ? reader.GetBoolean("general", "ignore_own_messages", true) : (strcmp((getenv("VANETZA_IGNORE_OWN_MESSAGES")), "true") == 0);
    config_s->ignore_rsu_messages =
        getenv("VANETZA_IGNORE_RSU_MESSAGES") == NULL ? reader.GetBoolean("general", "ignore_rsu_messages", false) : (strcmp((getenv("VANETZA_IGNORE_RSU_MESSAGES")), "true") == 0);
    config_s->enable_json_prints =
        getenv("VANETZA_ENABLE_JSON_PRINTS") == NULL ? reader.GetBoolean("general", "enable_json_prints", true) : (strcmp((getenv("VANETZA_ENABLE_JSON_PRINTS")), "true") == 0);
    ...
    config_s->publish_encoded_payloads =
        getenv("VANETZA_PUBLISH_ENCODED_PAYLOADS") == NULL ? reader.GetBoolean("general", "publish_encoded_payloads", false) : (strcmp((getenv("VANETZA_PUBLISH_ENCODED_PAYLOADS")), "true") == 0);
    ...
    // config_s->full_cam_topic_in = getenv("VANETZA_CAM_FULL_TOPIC_IN") == NULL ?
    // reader.Get("cam", "full_topic_in", "") :
    // getenv("VANETZA_CAM_FULL_TOPIC_IN"); config_s->full_cam_topic_out =
    // getenv("VANETZA_CAM_FULL_TOPIC_OUT") == NULL ? reader.Get("cam",
    // "full_topic_out", "") : getenv("VANETZA_CAM_FULL_TOPIC_OUT");
    // config_s->full_cam_topic_time = getenv("VANETZA_CAM_FULL_TOPIC_TIME") ==
    // NULL ? reader.Get("cam", "full_topic_time", "") :
    // getenv("VANETZA_CAM_FULL_TOPIC_TIME"); config_s->full_cam_topic_test =
    // getenv("VANETZA_CAM_FULL_TOPIC_TEST") == NULL ? reader.Get("cam",
    // "full_topic_test", "") : getenv("VANETZA_CAM_FULL_TOPIC_TEST");
    // config_s->full_vam_topic_in = getenv("VANETZA_VAM_FULL_TOPIC_IN") == NULL ?
    // reader.Get("vam", "full_topic_in", "") :
    // getenv("VANETZA_VAM_FULL_TOPIC_IN"); config_s->full_vam_topic_out =
    // getenv("VANETZA_VAM_FULL_TOPIC_OUT") == NULL ? reader.Get("vam",
    // "full_topic_out", "") : getenv("VANETZA_VAM_FULL_TOPIC_OUT");
    // config_s->full_vam_topic_time = getenv("VANETZA_VAM_FULL_TOPIC_TIME") ==
    // NULL ? reader.Get("vam", "full_topic_time", "") :
    // getenv("VANETZA_VAM_FULL_TOPIC_TIME"); config_s->full_vam_topic_test =
    // getenv("VANETZA_VAM_FULL_TOPIC_TEST") == NULL ? reader.Get("vam",
    // "full_topic_test", "") : getenv("VANETZA_VAM_FULL_TOPIC_TEST");
    ...
    config_s->tcpudp_enabled = getenv("VANETZA_TCP_ENABLED") == NULL ? reader.GetBoolean("tcpudp", "enabled", false) : (strcmp((getenv("VANETZA_TCP_ENABLED")), "true") == 0);
    // config_s->tcpudp_protocol = getenv("VANETZA_TCP_PROTOCOL") == NULL ? reader.Get("tcpudp", "protocol", "tcp") : getenv("VANETZA_TCP_PROTOCOL");
    config_s->tcpudp_interface = getenv("VANETZA_TCP_INTERFACE") == NULL ? reader.Get("tcpudp", "interface", "eth0") : getenv("VANETZA_TCP_INTERFACE");
    config_s->tcpudp_ip = getenv("VANETZA_TCP_IP") == NULL ? reader.Get("tcpudp", "ip", "127.0.0.1") : getenv("VANETZA_TCP_IP");
    config_s->tcpudp_port = getenv("VANETZA_TCP_PORT") == NULL ? reader.GetInteger("tcpudp", "port", 50050) : stoi(getenv("VANETZA_TCP_PORT"));
}
