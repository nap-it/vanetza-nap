...
void read_config(config_t* config_s, string path) {
    ...
    config_s->local_mqtt_broker = getenv("VANETZA_LOCAL_MQTT_BROKER") == NULL ? reader.Get("mqtt", "local_mqtt_broker", "127.0.0.1") : getenv("VANETZA_LOCAL_MQTT_BROKER");
    config_s->local_mqtt_port = getenv("VANETZA_LOCAL_MQTT_PORT") == NULL ? reader.GetInteger("mqtt", "local_mqtt_port", 1883) : stoi(getenv("VANETZA_LOCAL_MQTT_PORT"));
    config_s->remote_mqtt_broker = getenv("VANETZA_REMOTE_MQTT_BROKER") == NULL ? reader.Get("mqtt", "remote_mqtt_broker", "") : getenv("VANETZA_REMOTE_MQTT_BROKER");
    config_s->remote_mqtt_port = getenv("VANETZA_REMOTE_MQTT_PORT") == NULL ? reader.GetInteger("mqtt", "remote_mqtt_port", 0) : stoi(getenv("VANETZA_REMOTE_MQTT_PORT"));
    config_s->remote_mqtt_username = getenv("VANETZA_REMOTE_MQTT_USERNAME") == NULL ? reader.Get("mqtt", "remote_mqtt_username", "") : getenv("VANETZA_REMOTE_MQTT_USERNAME");
    config_s->remote_mqtt_password = getenv("VANETZA_REMOTE_MQTT_PASSWORD") == NULL ? reader.Get("mqtt", "remote_mqtt_password", "") : getenv("VANETZA_REMOTE_MQTT_PASSWORD");
    ...
    config_s->enable_json_prints = getenv("VANETZA_ENABLE_JSON_PRINTS") == NULL ? reader.GetBoolean("general", "enable_json_prints", false) : (strcmp((getenv("VANETZA_ENABLE_JSON_PRINTS")), "true") == 0);
    config_s->dds_enabled = getenv("VANETZA_DDS_ENABLED") == NULL ? reader.GetBoolean("dds", "dds_enabled", false) : (strcmp((getenv("VANETZA_DDS_ENABLED")), "true") == 0);
    config_s->dds_participant_name = getenv("VANETZA_DDS_PARTICIPANT_NAME") == NULL ? reader.Get("dds", "dds_participant_name", "Vanetza") : getenv("VANETZA_DDS_PARTICIPANT_NAME");
    config_s->dds_domain_id = getenv("VANETZA_DDS_DOMAIN_ID") == NULL ? reader.GetInteger("dds", "dds_domain_id", 0) : stoi(getenv("VANETZA_DDS_DOMAIN_ID"));
    config_s->dds_blocked_interfaces = getenv("VANETZA_DDS_BLOCKED_INTERFACES") == NULL ? reader.Get("dds", "dds_blocked_interfaces", "") : getenv("VANETZA_DDS_BLOCKED_INTERFACES");
    ...
    config_s->zenoh_local_only = getenv("VANETZA_ZENOH_LOCAL_ONLY") == NULL ? reader.GetBoolean("zenoh", "zenoh_local_only", true) : (strcmp((getenv("VANETZA_ZENOH_LOCAL_ONLY")), "true") == 0);
    config_s->zenoh_interfaces = getenv("VANETZA_ZENOH_INTERFACES") == NULL ? reader.Get("zenoh", "zenoh_interfaces", "") : getenv("VANETZA_ZENOH_INTERFACES");
    ...
    config_s->mim = read_message_config(reader, "VANETZA_MIM", "mim");
    config_s->mvm = read_message_config(reader, "VANETZA_MVM", "mvm");
    ...
    config_s->security = getenv("VANETZA_SECURITY") == NULL ? reader.Get("general", "security", "none") : getenv("VANETZA_SECURITY");
    config_s->ipv4_enabled = getenv("VANETZA_IPV4_ENABLED") == NULL ? reader.GetBoolean("ipv4", "enabled", false) : (strcmp((getenv("VANETZA_IPV4_ENABLED")), "true") == 0);
