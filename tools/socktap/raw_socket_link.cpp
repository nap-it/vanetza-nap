...
RawSocketLink::RawSocketLink(boost::asio::generic::raw_protocol::socket&& socket, const std::string& device_name, bool _rssi_enabled)
    ...
    if (setsockopt(socket_.native_handle(), SOL_SOCKET, SO_ATTACH_FILTER, &bpf_filter, sizeof(bpf_filter)) < 0) {
        std::cout << "Could not apply BPF filter to raw socket link. Proceeding "
                     "with unfiltered socket."
                  << std::endl;
    }
