...
#include "applications/mim_application.hpp"
#include "applications/mvm_application.hpp"
#include "applications/pmm_application.hpp"
...
int main(int argc, const char** argv)
{
    ...
    if(config_s.debug_enabled) std::cout << "Debug mode enabled. Verbose error messages will be printed for ASN.1 encoding/decoding errors." << std::endl;
    ...
    try {
		...
		if (config_s.ipv4_enabled) {
			...
			if (eth_layer) {
                ...
                std::cout << "ITS-G5 transport support enabled on interface " << config_s.interface << std::endl;
			}
			...
			if (ipv4_layer) {
                ...
                std::cout << "UDP transport support enabled on interface " << config_s.ipv4_interface << std::endl;
			}
        ...
        if (!link_layer) {
            std::cerr << "Error creating link layer, check configuration." << std::endl;
            return 1;
        }
		...
		if (config_s.mim.enabled) {
			std::unique_ptr<MimApplication> mim_app {
				new MimApplication(*positioning, context.get_dccp().get_trigger().runtime(), pubsub, config_s, metrics_s, 2, prom_mtx)
			};
			mim_app->set_interval(std::chrono::milliseconds(config_s.mim.periodicity));
			apps.emplace("mim", std::move(mim_app));
		}

		if (config_s.mvm.enabled) {
			std::unique_ptr<MvmApplication> mvm_app {
				new MvmApplication(*positioning, context.get_dccp().get_trigger().runtime(), pubsub, config_s, metrics_s, 2, prom_mtx)
			};
			mvm_app->set_interval(std::chrono::milliseconds(config_s.mvm.periodicity));
			apps.emplace("mvm", std::move(mvm_app));
		}

		if (config_s.pmm.enabled) {
			std::unique_ptr<PmmApplication> pmm_app {
				new PmmApplication(*positioning, context.get_dccp().get_trigger().runtime(), pubsub, config_s, metrics_s, 2, prom_mtx)
			};
			pmm_app->set_interval(std::chrono::milliseconds(config_s.pmm.periodicity));
			apps.emplace("pmm", std::move(pmm_app));
		}
