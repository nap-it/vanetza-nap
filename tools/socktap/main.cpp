...
int main(int argc, const char** argv) {
    ...
    options.add_options()("help", "Print out available options.")("config,c", po::value<std::string>()->default_value("config.ini"), "Config file path");
    ...
    try {
        ...
        if (mib.itsGnProtocolVersion != 0 && mib.itsGnProtocolVersion != 1) {
            throw std::runtime_error("Unsupported GeoNetworking version, only "
        ...
        metrics_s.packet_counter = &(BuildCounter().Name("observed_packets_count_total").Help("Number of observed packets").Register(*(metrics_s.registry)));

        metrics_s.latency_counter = &(BuildCounter().Name("observed_packets_latency_total").Help("Processing latency of observed packets").Register(*(metrics_s.registry)));
        ...
        if (config_s.tcpudp_enabled) {
            try {
                const char* transport_device_name = config_s.tcpudp_interface.c_str();
        ...
        if (config_s.cam.enabled) {
            std::unique_ptr<CamApplication> cam_app{new CamApplication(*positioning, context.get_dccp().get_trigger().runtime(), pubsub, config_s, metrics_s, get_router(num_threads), 1, prom_mtx)};
        ...
        if (config_s.denm.enabled) {
            std::unique_ptr<DenmApplication> denm_app{new DenmApplication(*positioning, context.get_dccp().get_trigger().runtime(), pubsub, config_s, metrics_s, 0, prom_mtx)};
        ...
        if (config_s.cpm.enabled) {
            std::unique_ptr<CpmApplication> cpm_app{new CpmApplication(*positioning, context.get_dccp().get_trigger().runtime(), pubsub, config_s, metrics_s, 0, prom_mtx)};
        ...
        if (config_s.vam.enabled) {
            std::unique_ptr<VamApplication> vam_app{new VamApplication(*positioning, context.get_dccp().get_trigger().runtime(), pubsub, config_s, metrics_s, 1, prom_mtx)};
        ...
        if (config_s.spatem.enabled) {
            std::unique_ptr<SpatemApplication> spatem_app{new SpatemApplication(*positioning, context.get_dccp().get_trigger().runtime(), pubsub, config_s, metrics_s, 1, prom_mtx)};
        ...
        if (config_s.mapem.enabled) {
            std::unique_ptr<MapemApplication> mapem_app{new MapemApplication(*positioning, context.get_dccp().get_trigger().runtime(), pubsub, config_s, metrics_s, 1, prom_mtx)};
        ...
        if (config_s.ssem.enabled) {
            std::unique_ptr<SsemApplication> ssem_app{new SsemApplication(*positioning, context.get_dccp().get_trigger().runtime(), pubsub, config_s, metrics_s, 2, prom_mtx)};
        ...
        if (config_s.srem.enabled) {
            std::unique_ptr<SremApplication> srem_app{new SremApplication(*positioning, context.get_dccp().get_trigger().runtime(), pubsub, config_s, metrics_s, 2, prom_mtx)};
        ...
        if (config_s.rtcmem.enabled) {
            std::unique_ptr<RtcmemApplication> rtcmem_app{new RtcmemApplication(*positioning, context.get_dccp().get_trigger().runtime(), pubsub, config_s, metrics_s, 2, prom_mtx)};
        ...
        if (config_s.ivim.enabled) {
            std::unique_ptr<IvimApplication> ivim_app{new IvimApplication(*positioning, context.get_dccp().get_trigger().runtime(), pubsub, config_s, metrics_s, 2, prom_mtx)};
        ...
        if (config_s.evcsnm.enabled) {
            std::unique_ptr<EvcsnmApplication> evcsnm_app{new EvcsnmApplication(*positioning, context.get_dccp().get_trigger().runtime(), pubsub, config_s, metrics_s, 2, prom_mtx)};
        ...
        if (config_s.evrsrm.enabled) {
            std::unique_ptr<EvrsrmApplication> evrsrm_app{new EvrsrmApplication(*positioning, context.get_dccp().get_trigger().runtime(), pubsub, config_s, metrics_s, 2, prom_mtx)};
        ...
        if (config_s.imzm.enabled) {
            std::unique_ptr<ImzmApplication> imzm_app{new ImzmApplication(*positioning, context.get_dccp().get_trigger().runtime(), pubsub, config_s, metrics_s, 2, prom_mtx)};
        ...
        if (config_s.tistpgm.enabled) {
            std::unique_ptr<TistpgmApplication> tistpgm_app{new TistpgmApplication(*positioning, context.get_dccp().get_trigger().runtime(), pubsub, config_s, metrics_s, 2, prom_mtx)};
        ...
        if (config_s.mcm.enabled) {
            std::unique_ptr<McmApplication> mcm_app{new McmApplication(*positioning, context.get_dccp().get_trigger().runtime(), pubsub, config_s, metrics_s, 1, prom_mtx)};
            mcm_app->set_interval(std::chrono::milliseconds(config_s.mcm.periodicity));
            apps.emplace("mcm", std::move(mcm_app));
        }

        if (apps.empty()) {
            std::cerr << "Warning: No applications are configured, only GN beacons "
                         "will be exchanged\n";
        }
