...
create_position_provider(boost::asio::io_context& io_context, const po::variables_map& vm, config_t config_s, const Runtime& runtime)
{
    ...
    if (!config_s.use_hardcoded_gps) {
        ...
        positioning.reset(new GpsPositionProvider {
            io_context, config_s.gpsd_host, config_s.gpsd_port
        });
#endif
    } else {
