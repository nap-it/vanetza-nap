...
RtcmemApplication::RtcmemApplication(PositionProvider& positioning, Runtime& rt, Mqtt *local_mqtt_, Mqtt *remote_mqtt_, Dds* dds_, config_t config_s_, metrics_t metrics_s_) :
    positioning_(positioning), runtime_(rt), rtcmem_interval_(seconds(1)), local_mqtt(local_mqtt_), remote_mqtt(remote_mqtt_), dds(dds_), config_s(config_s_), metrics_s(metrics_s_)
{
    if(config_s.rtcmem.mqtt_enabled) local_mqtt->subscribe(config_s.rtcmem.topic_in, this);
    if(config_s.rtcmem.mqtt_enabled && remote_mqtt != NULL) remote_mqtt->subscribe(config_s.remote_mqtt_prefix + std::to_string(config_s.station_id) + "/" + config_s.rtcmem.topic_in, this);
    if(config_s.rtcmem.dds_enabled) dds->subscribe(config_s.rtcmem.topic_in, this);
...
void RtcmemApplication::indicate(const DataIndication& indication, UpPacketPtr packet)
{
    ...
    rtcmem_json.Accept(writer);
    const char* rtcmemJSON = fullBuffer.GetString();

    if(config_s.rtcmem.udp_out_port != 0) {
        rtcmem_udp_socket.send_to(buffer(rtcmemJSON, strlen(rtcmemJSON)), rtcmem_remote_endpoint, 0, rtcmem_err);
    }
    const double time_full_udp = (double) duration_cast< microseconds >(system_clock::now().time_since_epoch()).count() / 1000000.0;
    if(config_s.rtcmem.dds_enabled) dds->publish(config_s.rtcmem.topic_out, rtcmemJSON);
    const double time_full_dds = (double) duration_cast< microseconds >(system_clock::now().time_since_epoch()).count() / 1000000.0;
    if(config_s.rtcmem.mqtt_enabled) local_mqtt->publish(config_s.rtcmem.topic_out, rtcmemJSON);
    const double time_full_local = (double) duration_cast< microseconds >(system_clock::now().time_since_epoch()).count() / 1000000.0;
    if(config_s.rtcmem.mqtt_enabled && remote_mqtt != NULL) remote_mqtt->publish(config_s.remote_mqtt_prefix + std::to_string(config_s.station_id) + "/" + config_s.rtcmem.topic_out, rtcmemJSON);
    const double time_full_remote = (double) duration_cast< microseconds >(system_clock::now().time_since_epoch()).count() / 1000000.0;
    
    rtcmem_rx_counter->Increment();
    rtcmem_rx_latency->Increment(time_full_remote - cp.time_received);

    if(config_s.rtcmem.mqtt_test_enabled) {
        Document::AllocatorType& allocator = rtcmem_json.GetAllocator();
        if (config_s.rtcmem.udp_out_port != 0) rtcmem_json["test"].AddMember("full_udp_timestamp", time_full_udp, allocator);
        if (config_s.rtcmem.dds_enabled != 0) rtcmem_json["test"].AddMember("full_dds_timestamp", time_full_dds, allocator);
        if (config_s.rtcmem.mqtt_enabled != 0) rtcmem_json["test"].AddMember("full_local_timestamp", time_full_local, allocator);
        if (config_s.rtcmem.mqtt_enabled && remote_mqtt != NULL) rtcmem_json["test"].AddMember("full_remote_timestamp", time_full_remote, allocator);

        StringBuffer testBuffer;
        Writer<StringBuffer> testWriter(testBuffer);
        rtcmem_json.Accept(testWriter);
        const char* testJSON = testBuffer.GetString();
        local_mqtt->publish(config_s.rtcmem.topic_test, testJSON);
        if(remote_mqtt != NULL) remote_mqtt->publish(config_s.remote_mqtt_prefix + std::to_string(config_s.station_id) + "/" + config_s.rtcmem.topic_test, testJSON);
    }

    if(config_s.enable_json_prints) std::cout << "rtcmem JSON: " << rtcmemJSON << std::endl;    
}
...
void RtcmemApplication::on_message(string topic, string mqtt_message) {
    ...
    if(config_s.rtcmem.mqtt_time_enabled) {
        ...
        local_mqtt->publish(config_s.rtcmem.topic_time, timeJSON);
        if (remote_mqtt != NULL) remote_mqtt->publish(config_s.remote_mqtt_prefix + std::to_string(config_s.station_id) + "/" + config_s.rtcmem.topic_time, timeJSON);
