...
void SpatemApplication::indicate(const DataIndication& indication, UpPacketPtr packet)
{
    ...
    rapidjson::Document spatem_doc;
    spatem_doc  = vanetza::asn1::rj::jer_encode_rj_doc(&asn_DEF_SPATEM, spatem->content());

    rapidjson::Document output_doc;
    auto& alloc = output_doc.GetAllocator();
    output_doc.SetObject();

    output_doc.AddMember("fields", spatem_doc, alloc);
    ...
    output_doc.AddMember("stationID", Value(static_cast<int64_t>((*spatem)->header.stationID)), alloc);
    ...
    pubsub->publish(config_s.spatem, output_doc, &spatem_udp_socket, &spatem_remote_endpoint, &spatem_err, spatem_rx_counter, spatem_rx_latency, cp.time_received, time_encoded, cp.time_queue, time_queue2, "SPATEM");
}
...
void SpatemApplication::on_message(string topic, string mqtt_message, const std::vector<uint8_t>& bytes, bool is_encoded, double time_reception, string test, vanetza::geonet::Router* router) {
    ...
    if (!is_encoded) {
        ...
        // Check if this is a full SPATEM (with header and spat) or just the spatem payload
        bool has_full_structure = document.HasMember("header") && document.HasMember("spat");

        if (!has_full_structure) {
            // Wrap the payload with header and spatem structure
            Document wrapper;
            ...
            header.AddMember("protocolVersion", 2, allocator)
                .AddMember("messageID", ItsPduHeader__messageID_spatem, allocator)
            ...
            // Move the document content under "spat"
            Value spatemContent;
            spatemContent.CopyFrom(document, allocator);
            wrapper.AddMember("spat", spatemContent, allocator);
        ...
        packet->layer(OsiLayer::Application) = std::move(spatem);
    } else {
