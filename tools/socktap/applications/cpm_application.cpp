...
#include <vanetza/asn1/support/jer_encoder.h>
...
void CpmApplication::indicate(const DataIndication& indication, UpPacketPtr packet)
{
    ...
    asn1::PacketVisitor<asn1::r2::Cpm> visitor;
    std::shared_ptr<const asn1::r2::Cpm> cpm = boost::apply_visitor(visitor, *packet);
    if (cpm == 0) {
    ...
    // Debug: use original jer_fprint to print received CPM structure
    // std::cout << "=== DEBUG: Received CPM (JER encoded) ===" << std::endl;
    // jer_fprint(stdout, &asn_DEF_Vanetza_ITS2_CollectivePerceptionMessage, cpm->content());
    // std::cout << "\n=========================================" << std::endl;
    ...
    rapidjson::Document cpm_doc;
    cpm_doc  = vanetza::asn1::rj::jer_encode_rj_doc(&asn_DEF_Vanetza_ITS2_CollectivePerceptionMessage, cpm->content());
    
    rapidjson::Document output_doc;
    auto& alloc = output_doc.GetAllocator();
    output_doc.SetObject();

    output_doc.AddMember("fields", cpm_doc, alloc);
    ...
    output_doc.AddMember("stationID", Value(static_cast<int64_t>((*cpm)->header.stationId)), alloc);
    ...
    pubsub->publish(config_s.cpm, output_doc, &cpm_udp_socket, &cpm_remote_endpoint, &cpm_err, cpm_rx_counter, cpm_rx_latency, cp.time_received, time_encoded, cp.time_queue, time_queue2, "CPM");
}
...
void CpmApplication::on_message(string topic, string mqtt_message, const std::vector<uint8_t>& bytes, bool is_encoded, double time_reception, string test, vanetza::geonet::Router* router) {
    ...
    if (!is_encoded) {
        ...
        // Check if this is a full CPM (with header and payload) or just the cpm payload
        bool has_full_structure = document.HasMember("header") && document.HasMember("payload");
        ...
        if (!has_full_structure) {
            ...
            header.AddMember("protocolVersion", 2, allocator)
                .AddMember("messageId", Vanetza_ITS2_MessageId_cpm, allocator)
            ...
            // Move the document content under "payload"
            Value cpmContent;
            cpmContent.CopyFrom(document, allocator);
            wrapper.AddMember("payload", cpmContent, allocator);
        ...
        packet->layer(OsiLayer::Application) = std::move(cpm);
    } else {
