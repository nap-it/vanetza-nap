...
void EvcsnmApplication::indicate(const DataIndication& indication, UpPacketPtr packet)
{
    ...
    Document evcsnm_json = buildJSON(evcsnm_t, cp.time_received, cp.rssi, cp.size(), cp.time_queue);
...
Document EvcsnmApplication::buildJSON(EvcsnPdu_t message, double time_reception, int rssi, int packet_size, double time_queue) {
