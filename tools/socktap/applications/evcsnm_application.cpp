...
void EvcsnmApplication::indicate(const DataIndication& indication, UpPacketPtr packet)
{
    ...
    Document evcsnm_json = buildJSON(evcsnm_t, cp.time_received, cp.rssi, cp.size(), cp.time_queue, parse_channel_info(cp), cp.source);
...
Document EvcsnmApplication::buildJSON(EvcsnPdu_t message, double time_reception, int rssi, int packet_size, double time_queue, channel channel_info, string source) {
...
void EvcsnmApplication::on_message(string topic, string mqtt_message, const std::vector<uint8_t>& bytes, bool is_encoded, double time_reception, string test, vanetza::geonet::Router* router) {
    ...
    if (!is_encoded) {
        ...
        header.messageID = MessageId_evcsn;
