...
void MapemApplication::indicate(const DataIndication& indication, UpPacketPtr packet)
{
    ...
    rapidjson::Document mapem_doc;
    mapem_doc  = vanetza::asn1::rj::jer_encode_rj_doc(&asn_DEF_MAPEM, mapem->content());

    rapidjson::Document output_doc;
    auto& alloc = output_doc.GetAllocator();
    output_doc.SetObject();

    output_doc.AddMember("fields", mapem_doc, alloc);
    ...
    output_doc.AddMember("stationID", Value(static_cast<int64_t>((*mapem)->header.stationID)), alloc);
    ...
    pubsub->publish(config_s.mapem, output_doc, &mapem_udp_socket, &mapem_remote_endpoint, &mapem_err, mapem_rx_counter, mapem_rx_latency, cp.time_received, time_encoded, cp.time_queue, time_queue2, "MAPEM");
}
...
void MapemApplication::on_message(string topic, string mqtt_message, const std::vector<uint8_t>& bytes, bool is_encoded, double time_reception, string test, vanetza::geonet::Router* router) {
    ...
    if (!is_encoded) {
        ...
        // Check if this is a full MAPEM (with header and map) or just the mapem payload
        bool has_full_structure = document.HasMember("header") && document.HasMember("map");

        if (!has_full_structure) {
            // Wrap the payload with header and mapem structure
            Document wrapper;
            ...
            header.AddMember("protocolVersion", 2, allocator)
                .AddMember("messageID", ItsPduHeader__messageID_mapem, allocator)
            ...
            // Move the document content under "map"
            Value mapemContent;
            mapemContent.CopyFrom(document, allocator);
            wrapper.AddMember("map", mapemContent, allocator);
        ...
        packet->layer(OsiLayer::Application) = std::move(mapem);
    } else {
