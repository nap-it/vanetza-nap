...
void MapemApplication::indicate(const DataIndication& indication, UpPacketPtr packet)
{
    ...
    Document mapem_json = buildJSON(mapem_t, cp.time_received, cp.rssi, cp.size(), cp.time_queue);
...
Document MapemApplication::buildJSON(MAPEM_t message, double time_reception, int rssi, int packet_size, double time_queue) {
