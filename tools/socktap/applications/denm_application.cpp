...
void DenmApplication::indicate(const DataIndication& indication, UpPacketPtr packet)
{
    ...
    asn1::PacketVisitor<asn1::r2::Denm> visitor;
    std::shared_ptr<const asn1::r2::Denm> denm = boost::apply_visitor(visitor, *packet);
    if (denm == 0) {
    ...
    rapidjson::Document denm_doc;
    denm_doc  = vanetza::asn1::rj::jer_encode_rj_doc(&asn_DEF_Vanetza_ITS2_DENM, denm->content());

    rapidjson::Document output_doc;
    auto& alloc = output_doc.GetAllocator();
    output_doc.SetObject();

    output_doc.AddMember("fields", denm_doc, alloc);
    ...
    output_doc.AddMember("stationID", Value(static_cast<int64_t>((*denm)->header.stationId)), alloc);
    ...
    pubsub->publish(config_s.denm, output_doc, &denm_udp_socket, &denm_remote_endpoint, &denm_err, denm_rx_counter, denm_rx_latency, cp.time_received, time_encoded, cp.time_queue, time_queue2, "DENM");
}
...
void DenmApplication::on_message(string topic, string mqtt_message, const std::vector<uint8_t>& bytes, bool is_encoded, double time_reception, string test, vanetza::geonet::Router* router) {
    ...
    if (!is_encoded) {
        ...
        // Check if this is a full DENM (with header and denm) or just the denm payload
        bool has_full_structure = document.HasMember("header") && document.HasMember("denm");

        if (!has_full_structure) {
            // Wrap the payload with header and denm structure
            Document wrapper;
            ...
            header.AddMember("protocolVersion", 2, allocator)
                .AddMember("messageId", Vanetza_ITS2_MessageId_denm, allocator)
            ...
            // Move the document content under "denm"
            Value denmContent;
            denmContent.CopyFrom(document, allocator);
            wrapper.AddMember("denm", denmContent, allocator);
        ...
        packet->layer(OsiLayer::Application) = std::move(denm);
    } else {
