...
prometheus::Counter *mvm_rx_counter;
prometheus::Counter *mvm_tx_counter;
prometheus::Counter *mvm_rx_latency;
prometheus::Counter *mvm_tx_latency;

boost::asio::io_service mvm_io_service_;
ip::udp::socket mvm_udp_socket(mvm_io_service_);
ip::udp::endpoint mvm_remote_endpoint;
boost::system::error_code mvm_err;

MvmApplication::MvmApplication(PositionProvider& positioning, Runtime& rt, PubSub* pubsub_, config_t config_s_, metrics_t metrics_s_, int priority_, std::mutex& prom_mtx_) :
    PubSub_application(priority_),
    positioning_(positioning), runtime_(rt), mvm_interval_(seconds(1)), pubsub(pubsub_), config_s(config_s_), metrics_s(metrics_s_), priority(priority_), prom_mtx(prom_mtx_)
{
    mvm_rx_counter = &((*metrics_s.packet_counter).Add({{"message", "mvm"}, {"direction", "rx"}}));
    mvm_tx_counter = &((*metrics_s.packet_counter).Add({{"message", "mvm"}, {"direction", "tx"}}));
    mvm_rx_latency = &((*metrics_s.latency_counter).Add({{"message", "mvm"}, {"direction", "rx"}}));
    mvm_tx_latency = &((*metrics_s.latency_counter).Add({{"message", "mvm"}, {"direction", "tx"}}));

    this->pubsub->subscribe(config_s.mvm, this);

    if(config_s.mvm.udp_out_port != 0) {
        mvm_udp_socket.open(ip::udp::v4());
        mvm_remote_endpoint = ip::udp::endpoint(ip::address::from_string(config_s.mvm.udp_out_addr), config_s.mvm.udp_out_port);
    }
}

void MvmApplication::set_interval(Clock::duration interval)
{
    mvm_interval_ = interval;
    runtime_.cancel(this);
    if (interval != std::chrono::milliseconds(0)) schedule_timer();
}

MvmApplication::PortType MvmApplication::port()
{
	return btp::ports::MVM;
}

void MvmApplication::indicate(const DataIndication& indication, UpPacketPtr packet)
{
    ...
    asn1::PacketVisitor<asn1::r2::Mvm> visitor;
    std::shared_ptr<const asn1::r2::Mvm> mvm = boost::apply_visitor(visitor, *packet);
    if (mvm == 0) {
        std::cout << "-- Vanetza Decoding Error --\nReceived an encoded MVM message that does not meet ETSI spec" << std::endl;
    ...
    rapidjson::Document mvm_doc;
    mvm_doc  = vanetza::asn1::rj::jer_encode_rj_doc(&asn_DEF_Vanetza_ITS2_MVM, mvm->content());

    rapidjson::Document output_doc;
    auto& alloc = output_doc.GetAllocator();
    output_doc.SetObject();

    output_doc.AddMember("fields", mvm_doc, alloc);
    ...
    output_doc.AddMember("stationID", Value(static_cast<int64_t>((*mvm)->header.stationId)), alloc);
    ...
    pubsub->publish(config_s.mvm, output_doc, &mvm_udp_socket, &mvm_remote_endpoint, &mvm_err, mvm_rx_counter, mvm_rx_latency, cp.time_received, time_encoded, cp.time_queue, time_queue2, "mvm");
}

void MvmApplication::schedule_timer()
{
    runtime_.schedule(mvm_interval_, std::bind(&MvmApplication::on_timer, this, std::placeholders::_1), this);
}

void MvmApplication::on_message(string topic, string mqtt_message, const std::vector<uint8_t>& bytes, bool is_encoded, double time_reception, string test, vanetza::geonet::Router* router) {
    ...
    if (!is_encoded) {
        ...
        // Check if this is a full MVM (with header and payload) or just the mvm payload
        bool has_full_structure = document.HasMember("header") && document.HasMember("mvm");
        ...
        if (!has_full_structure) {
			...
			header.AddMember("protocolVersion", 3, allocator)
				.AddMember("messageId", Vanetza_ITS2_MessageId_pcvm, allocator)
			...
			if (document.HasMember("e2eProtection")) {
				Value e2e_protection;
				e2e_protection.CopyFrom(document["e2eProtection"], allocator);
				wrapper.AddMember("e2eProtection", e2e_protection, allocator);
			} else {
				std::cout << "Expected e2eProtection field not found in the document." << std::endl;
				std::cout << "Invalid payload: " << mqtt_message << std::endl;
				return;
			}

            // Move the document content under "mvm"
            Value mvmContent;
            mvmContent.CopyFrom(document["mvm"], allocator);
            wrapper.AddMember("mvm", mvmContent, allocator);
        ...
        packet->layer(OsiLayer::Application) = std::move(mvm);
    } else {
    ...
    if(config_s.mvm.mqtt_time_enabled) {
        ...
        pubsub->publish_time(config_s.mvm, timePayload);
    }

    const double time_now = (double) duration_cast< microseconds >(system_clock::now().time_since_epoch()).count() / 1000000.0;
    prom_mtx.lock();
    mvm_tx_counter->Increment();
    mvm_tx_latency->Increment(time_now - time_reception);
    prom_mtx.unlock();
}

void MvmApplication::on_timer(Clock::time_point)
{
