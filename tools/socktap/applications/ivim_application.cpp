...
void IvimApplication::indicate(const DataIndication& indication, UpPacketPtr packet)
{
    ...
    Document ivim_json = buildJSON(ivim_t, cp.time_received, cp.rssi, cp.size(), cp.time_queue);
...
Document IvimApplication::buildJSON(IVIM_t message, double time_reception, int rssi, int packet_size, double time_queue) {
