...
void ImzmApplication::indicate(const DataIndication& indication, UpPacketPtr packet)
{
    ...
    Document imzm_json = buildJSON(imzm_t, cp.time_received, cp.rssi, cp.size(), cp.time_queue, parse_channel_info(cp), cp.source);
...
Document ImzmApplication::buildJSON(IMZM_t message, double time_reception, int rssi, int packet_size, double time_queue, channel channel_info, string source) {
