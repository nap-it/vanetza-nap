...
void McmApplication::indicate(const DataIndication& indication, UpPacketPtr packet)
{
    ...
    rapidjson::Document mcm_doc;
    mcm_doc  = vanetza::asn1::rj::jer_encode_rj_doc(&asn_DEF_Vanetza_ITS2_MCM, mcm->content());

    rapidjson::Document output_doc;
    auto& alloc = output_doc.GetAllocator();
    output_doc.SetObject();

    output_doc.AddMember("fields", mcm_doc, alloc);
    ...
    output_doc.AddMember("stationID", Value(static_cast<int64_t>((*mcm)->header.stationId)), alloc);
    ...
    pubsub->publish(config_s.mcm, output_doc, &mcm_udp_socket, &mcm_remote_endpoint, &mcm_err, mcm_rx_counter, mcm_rx_latency, cp.time_received, time_encoded, cp.time_queue, time_queue2, "MCM");
}
...
void McmApplication::on_message(string topic, string mqtt_message, const std::vector<uint8_t>& bytes, bool is_encoded, double time_reception, string test, vanetza::geonet::Router* router) {
    ...
    if (!is_encoded) {
        ...
        // Check if this is a full MCM (with header and mcm) or just the mcm payload
        bool has_full_structure = document.HasMember("header") && document.HasMember("mcm");

        if (!has_full_structure) {
            // Wrap the payload with header and mcm structure
            Document wrapper;
            ...
            header.AddMember("protocolVersion", 2, allocator)
                .AddMember("messageId", Vanetza_ITS2_MessageId_mcm, allocator)
            ...
            // Move the document content under "mcm"
            Value mcmContent;
            mcmContent.CopyFrom(document, allocator);
            wrapper.AddMember("payload", mcmContent, allocator);
        ...
        packet->layer(OsiLayer::Application) = std::move(mcm);
    } else {
