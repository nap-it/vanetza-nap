...
void SremApplication::indicate(const DataIndication& indication, UpPacketPtr packet)
{
    ...
    rapidjson::Document srem_doc;
    srem_doc  = vanetza::asn1::rj::jer_encode_rj_doc(&asn_DEF_SREM, srem->content());

    rapidjson::Document output_doc;
    auto& alloc = output_doc.GetAllocator();
    output_doc.SetObject();

    output_doc.AddMember("fields", srem_doc, alloc);
    ...
    output_doc.AddMember("stationID", Value(static_cast<int64_t>((*srem)->header.stationID)), alloc);
    ...
    pubsub->publish(config_s.srem, output_doc, &srem_udp_socket, &srem_remote_endpoint, &srem_err, srem_rx_counter, srem_rx_latency, cp.time_received, time_encoded, cp.time_queue, time_queue2, "SREM");
}
...
void SremApplication::on_message(string topic, string mqtt_message, const std::vector<uint8_t>& bytes, bool is_encoded, double time_reception, string test, vanetza::geonet::Router* router) {
    ...
    if (!is_encoded) {
        ...
        // Check if this is a full SREM (with header and srm) or just the srem payload
        bool has_full_structure = document.HasMember("header") && document.HasMember("srm");

        if (!has_full_structure) {
            // Wrap the payload with header and srem structure
            Document wrapper;
            ...
            header.AddMember("protocolVersion", 2, allocator)
                .AddMember("messageID", ItsPduHeader__messageID_srem, allocator)
            ...
            // Move the document content under "srm"
            Value sremContent;
            sremContent.CopyFrom(document, allocator);
            wrapper.AddMember("srm", sremContent, allocator);
        ...
        packet->layer(OsiLayer::Application) = std::move(srem);
    } else {
