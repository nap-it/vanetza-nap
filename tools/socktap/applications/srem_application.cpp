...
void SremApplication::indicate(const DataIndication& indication, UpPacketPtr packet)
{
    ...
    Document srem_json = buildJSON(srem_t, cp.time_received, cp.rssi, cp.size(), cp.time_queue, parse_channel_info(cp), cp.source);
...
Document SremApplication::buildJSON(SREM_t message, double time_reception, int rssi, int packet_size, double time_queue, channel channel_info, string source) {
...
void SremApplication::on_message(string topic, string mqtt_message, const std::vector<uint8_t>& bytes, bool is_encoded, double time_reception, string test, vanetza::geonet::Router* router) {
    ...
    if (!is_encoded) {
        ...
        header.messageID = MessageId_srem;
