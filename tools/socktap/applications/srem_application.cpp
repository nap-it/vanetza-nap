...
void SremApplication::indicate(const DataIndication& indication, UpPacketPtr packet)
{
    ...
    Document srem_json = buildJSON(srem_t, cp.time_received, cp.rssi, cp.size(), cp.time_queue);
...
Document SremApplication::buildJSON(SREM_t message, double time_reception, int rssi, int packet_size, double time_queue) {
