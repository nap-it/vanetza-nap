...
void RtcmemApplication::indicate(const DataIndication& indication, UpPacketPtr packet)
{
    ...
    if (rtcmem == 0) {
        std::cout << "-- Vanetza Decoding Error --\nReceived an encoded RTCMEM message that does not meet ETSI spec" << std::endl;
    ...
    rapidjson::Document rtcmem_doc;
    rtcmem_doc  = vanetza::asn1::rj::jer_encode_rj_doc(&asn_DEF_RTCMEM, rtcmem->content());

    rapidjson::Document output_doc;
    auto& alloc = output_doc.GetAllocator();
    output_doc.SetObject();

    output_doc.AddMember("fields", rtcmem_doc, alloc);
    ...
    output_doc.AddMember("stationID", Value(static_cast<int64_t>((*rtcmem)->header.stationID)), alloc);
    ...
    pubsub->publish(config_s.rtcmem, output_doc, &rtcmem_udp_socket, &rtcmem_remote_endpoint, &rtcmem_err, rtcmem_rx_counter, rtcmem_rx_latency, cp.time_received, time_encoded, cp.time_queue, time_queue2, "RTCMEM");
}
...
void RtcmemApplication::on_message(string topic, string mqtt_message, const std::vector<uint8_t>& bytes, bool is_encoded, double time_reception, string test, vanetza::geonet::Router* router) {
    ...
    if (!is_encoded) {
        ...
        // Check if this is a full RTCMEM (with header and rtcmc) or just the rtcmem payload
        bool has_full_structure = document.HasMember("header") && document.HasMember("rtcmc");

        if (!has_full_structure) {
            // Wrap the payload with header and rtcmem structure
            Document wrapper;
            ...
            header.AddMember("protocolVersion", 2, allocator)
                .AddMember("messageID", ItsPduHeader__messageID_rtcmem, allocator)
            ...
            // Move the document content under "rtcmc"
            Value rtcmemContent;
            rtcmemContent.CopyFrom(document, allocator);
            wrapper.AddMember("rtcmc", rtcmemContent, allocator);
        ...
        packet->layer(OsiLayer::Application) = std::move(rtcmem);
    } else {
