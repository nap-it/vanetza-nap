    ...
    positioning_(positioning), runtime_(rt), vam_interval_(seconds(1)), pubsub(pubsub_), config_s(config_s_), metrics_s(metrics_s_), priority(priority_), prom_mtx(prom_mtx_)
{
    ...
    this->pubsub->subscribe(config_s.vam, this);
...
void VamApplication::indicate(const DataIndication& indication, UpPacketPtr packet)
{
    ...
    asn1::PacketVisitor<asn1::r2::Vam> visitor;
    std::shared_ptr<const asn1::r2::Vam> vam = boost::apply_visitor(visitor, *packet);
    if (vam == 0) {
    ...
    rapidjson::Document vam_doc;
    vam_doc  = vanetza::asn1::rj::jer_encode_rj_doc(&asn_DEF_Vanetza_ITS2_VAM, vam->content());

    rapidjson::Document output_doc;
    auto& alloc = output_doc.GetAllocator();
    output_doc.SetObject();

    output_doc.AddMember("fields", vam_doc, alloc);
    ...
    output_doc.AddMember("stationID", Value(static_cast<int64_t>((*vam)->header.stationId)), alloc);
    ...
    pubsub->publish(config_s.vam, output_doc, &vam_udp_socket, &vam_remote_endpoint, &vam_err, vam_rx_counter, vam_rx_latency, cp.time_received, time_encoded, cp.time_queue, time_queue2, "VAM");
}
...
void VamApplication::on_message(string topic, string mqtt_message, const std::vector<uint8_t>& bytes, bool is_encoded, double time_reception, string test, vanetza::geonet::Router* router) {
    ...
    if (!is_encoded) {
        ...
        // Check if this is a full VAM (with header and payload) or just the vam payload
        bool has_full_structure = document.HasMember("header") && document.HasMember("vam");
        ...
        if (!has_full_structure) {
            ...
            header.AddMember("protocolVersion", 3, allocator)
                .AddMember("messageId", Vanetza_ITS2_MessageId_vam, allocator)
            ...
            // Move the document content under "vam"
            Value vamContent;
            vamContent.CopyFrom(document, allocator);
            wrapper.AddMember("vam", vamContent, allocator);
        ...
        packet->layer(OsiLayer::Application) = std::move(vam);
    } else {
