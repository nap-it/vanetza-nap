...
prometheus::Counter *mim_rx_counter;
prometheus::Counter *mim_tx_counter;
prometheus::Counter *mim_rx_latency;
prometheus::Counter *mim_tx_latency;

boost::asio::io_service mim_io_service_;
ip::udp::socket mim_udp_socket(mim_io_service_);
ip::udp::endpoint mim_remote_endpoint;
boost::system::error_code mim_err;

MimApplication::MimApplication(PositionProvider& positioning, Runtime& rt, PubSub* pubsub_, config_t config_s_, metrics_t metrics_s_, int priority_, std::mutex& prom_mtx_) :
    PubSub_application(priority_),
    positioning_(positioning), runtime_(rt), mim_interval_(seconds(1)), pubsub(pubsub_), config_s(config_s_), metrics_s(metrics_s_), priority(priority_), prom_mtx(prom_mtx_)
{
    mim_rx_counter = &((*metrics_s.packet_counter).Add({{"message", "mim"}, {"direction", "rx"}}));
    mim_tx_counter = &((*metrics_s.packet_counter).Add({{"message", "mim"}, {"direction", "tx"}}));
    mim_rx_latency = &((*metrics_s.latency_counter).Add({{"message", "mim"}, {"direction", "rx"}}));
    mim_tx_latency = &((*metrics_s.latency_counter).Add({{"message", "mim"}, {"direction", "tx"}}));

    this->pubsub->subscribe(config_s.mim, this);

    if(config_s.mim.udp_out_port != 0) {
        mim_udp_socket.open(ip::udp::v4());
        mim_remote_endpoint = ip::udp::endpoint(ip::address::from_string(config_s.mim.udp_out_addr), config_s.mim.udp_out_port);
    }
}

void MimApplication::set_interval(Clock::duration interval)
{
    mim_interval_ = interval;
    runtime_.cancel(this);
    if (interval != std::chrono::milliseconds(0)) schedule_timer();
}

MimApplication::PortType MimApplication::port()
{
    return btp::ports::MIM;
}

void MimApplication::indicate(const DataIndication& indication, UpPacketPtr packet)
{
    ...
    asn1::PacketVisitor<asn1::r2::Mim> visitor;
    std::shared_ptr<const asn1::r2::Mim> mim = boost::apply_visitor(visitor, *packet);
    if (mim == 0) {
        std::cout << "-- Vanetza Decoding Error --\nReceived an encoded MIM message that does not meet ETSI spec" << std::endl;
    ...
    rapidjson::Document mim_doc;
    mim_doc  = vanetza::asn1::rj::jer_encode_rj_doc(&asn_DEF_Vanetza_ITS2_MIM, mim->content());

    rapidjson::Document output_doc;
    auto& alloc = output_doc.GetAllocator();
    output_doc.SetObject();

    output_doc.AddMember("fields", mim_doc, alloc);
    ...
    output_doc.AddMember("stationID", Value(static_cast<int64_t>((*mim)->header.stationId)), alloc);
    ...
    pubsub->publish(config_s.mim, output_doc, &mim_udp_socket, &mim_remote_endpoint, &mim_err, mim_rx_counter, mim_rx_latency, cp.time_received, time_encoded, cp.time_queue, time_queue2, "mim");
}

void MimApplication::schedule_timer()
{
    runtime_.schedule(mim_interval_, std::bind(&MimApplication::on_timer, this, std::placeholders::_1), this);
}

void MimApplication::on_message(string topic, string mqtt_message, const std::vector<uint8_t>& bytes, bool is_encoded, double time_reception, string test, vanetza::geonet::Router* router) {
    ...
    if (!is_encoded) {
        ...
        // Check if this is a full MIM (with header and payload) or just the mim payload
        bool has_full_structure = document.HasMember("header") && document.HasMember("mim");
        ...
        if (!has_full_structure) {
            ...
            // Move the document content under "mims"
            Value mimContent;
            mimContent.CopyFrom(document["mims"], allocator);
            wrapper.AddMember("mims", mimContent, allocator);
        ...
        packet->layer(OsiLayer::Application) = std::move(mim);
    } else {
    ...
    if(config_s.mim.mqtt_time_enabled) {
        ...
        pubsub->publish_time(config_s.mim, timePayload);
    }

    const double time_now = (double) duration_cast< microseconds >(system_clock::now().time_since_epoch()).count() / 1000000.0;
    prom_mtx.lock();
    mim_tx_counter->Increment();
    mim_tx_latency->Increment(time_now - time_reception);
    prom_mtx.unlock();
}

void MimApplication::on_timer(Clock::time_point)
{
