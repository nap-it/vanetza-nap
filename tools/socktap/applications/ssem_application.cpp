...
void SsemApplication::indicate(const DataIndication& indication, UpPacketPtr packet)
{
    ...
    rapidjson::Document ssem_doc;
    ssem_doc  = vanetza::asn1::rj::jer_encode_rj_doc(&asn_DEF_SSEM, ssem->content());

    rapidjson::Document output_doc;
    auto& alloc = output_doc.GetAllocator();
    output_doc.SetObject();

    output_doc.AddMember("fields", ssem_doc, alloc);
    ...
    output_doc.AddMember("stationID", Value(static_cast<int64_t>((*ssem)->header.stationID)), alloc);
    ...
    pubsub->publish(config_s.ssem, output_doc, &ssem_udp_socket, &ssem_remote_endpoint, &ssem_err, ssem_rx_counter, ssem_rx_latency, cp.time_received, time_encoded, cp.time_queue, time_queue2, "SSEM");
}
...
void SsemApplication::on_message(string topic, string mqtt_message, const std::vector<uint8_t>& bytes, bool is_encoded, double time_reception, string test, vanetza::geonet::Router* router) {
    ...
    if (!is_encoded) {
        ...
        // Check if this is a full SSEM (with header and ssem) or just the ssem payload
        bool has_full_structure = document.HasMember("header") && document.HasMember("ssm");

        if (!has_full_structure) {
            // Wrap the payload with header and ssem structure
            Document wrapper;
            ...
            header.AddMember("protocolVersion", 2, allocator)
                .AddMember("messageID", ItsPduHeader__messageID_ssem, allocator)
            ...
            // Move the document content under "ssm"
            Value ssemContent;
            ssemContent.CopyFrom(document, allocator);
            wrapper.AddMember("ssm", ssemContent, allocator);
        ...
        packet->layer(OsiLayer::Application) = std::move(ssem);
    } else {
