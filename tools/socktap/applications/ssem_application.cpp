...
void SsemApplication::indicate(const DataIndication& indication, UpPacketPtr packet)
{
    ...
    Document ssem_json = buildJSON(ssem_t, cp.time_received, cp.rssi, cp.size(), cp.time_queue, parse_channel_info(cp));
...
Document SsemApplication::buildJSON(SSEM_t message, double time_reception, int rssi, int packet_size, double time_queue, channel channel_info) {
