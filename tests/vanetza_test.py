...
def extract_payload(fields: Dict[str, Any], message_type: str) -> Dict[str, Any]:
    """Extract the echo payload from a vanetza 'fields' object.

    Rebuilds a structure that mirrors the original in_<type>.json file:
    - Skips the 'header' wrapper added by vanetza.
    - Picks the content key: either equal to message_type (e.g. 'cam', 'mvm') or
      the first non-wrapper key (e.g. 'ivi' for ivim topic, 'mims' for mim topic).
    - For messages with 'e2eProtection' (MIM/MVM), returns
      {content_key: value, e2eProtection: value} to match in_<type>.json layout.
    - For messages without e2eProtection (CAM, DENM, IVIM, ...), returns the
      content value unwrapped, matching the raw in_<type>.json layout.
    """
    print(
        f"[extract_payload] message_type={message_type!r} "
    ...
    if message_type in fields:
        content_key = message_type
    else:
        skip_keys = {"header", "e2eProtection"}
        content_keys = [k for k in fields.keys() if k not in skip_keys]
        if not content_keys:
            raise ValueError(
                f"No payload content found in fields for message type '{message_type}'"
            )
        content_key = content_keys[0]

    content_value = fields[content_key]

    # MIM/MVM case: preserve e2eProtection alongside the content
    if "e2eProtection" in fields:
        return {
            content_key: content_value,
            "e2eProtection": fields["e2eProtection"],
        }
...
def sender_on_message(client, userdata, msg):
    ...
    try:
        ...
        if received_station_id == station_id:
            ...
            # Extract message data (preserves e2eProtection for MIM/MVM)
            received_cam = extract_payload(obj["fields"], message_type)
...
def receiver_on_message(client, userdata, msg):
    ...
    try:
        ...
        # Extract message data and echo it back (preserves e2eProtection for MIM/MVM)
        echo_payload = extract_payload(data["fields"], message_type)
        print("Echoing message type:", message_type)
        response_payload = json.dumps(echo_payload)
...
def test_single_message(file_path: str, args, mqtt_client) -> Dict[str, Any]:
    ...
    try:
        ...
        if not received:
            result["error"] = f"Timeout - no echo received"
...
def run_batch(args):
    ...
    for i, file_path in enumerate(message_files):
        ...
        if result["error"] == "Timeout - no echo received":
            second_try_result = test_single_message(file_path, args, mqtt_client)
            print("second try result:", second_try_result)
            results.append(second_try_result)
        else:
