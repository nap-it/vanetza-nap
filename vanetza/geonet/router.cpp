...
struct ControlInfo
{
    ControlInfo(const DataRequest& request) :
        ...
        source_mac_override(request.source_mac_override)
    {}

    const CommunicationProfile communication_profile;
    const ItsAid its_aid;
    const ByteBuffer permissions;
    const boost::optional<MacAddress> source_mac_override;
};
...
DataConfirm Router::request(const ShbDataRequest& request, DownPacketPtr payload)
{
    ...
    if (result.accepted()) {
        ...
        auto transmit = [this, ctrl](PendingPacket::Packet&& packet) {
            ...
            pass_down(cBroadcastMacAddress, std::move(pdu), std::move(payload), ctrl.source_mac_override);
...
DataConfirm Router::request(const GbcDataRequest& request, DownPacketPtr payload)
{
    ...
    auto transmit = [this, ctrl](Packet&& packet, const MacAddress& mac) {
        ...
        pass_down(mac, std::move(pdu), std::move(payload), ctrl.source_mac_override);
    };
...
void Router::indicate_secured(IndicationContextBasic& ctx, const BasicHeader& basic)
{
    ...
    if (!secured_message) {
        ...
        // No security entity, but Non_Strict mode allows processing secured packets without verification
        // Extract payload directly from secured message and pass through without decapsulation
        using namespace vanetza::security;
        PacketVariant payload = get_payload_copy(SecuredMessageView { *secured_message });
        ctx.service_primitive().security_report = VerificationReport::Verification_Skipped; // secured but not verified
        ctx.service_primitive().its_aid = get_its_aid(SecuredMessageView { *secured_message });
        secured_payload_visitor visitor(*this, ctx, basic);
                       ...
                       const boost::optional<MacAddress>& source_mac_override)
{
    ...
    request.source = source_mac_override ? *source_mac_override : m_local_position_vector.gn_addr.mid();
    request.source_override = source_mac_override;
