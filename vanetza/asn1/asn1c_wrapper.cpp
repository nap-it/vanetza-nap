...
bool decode_per_debug(asn_TYPE_descriptor_t& td, void** t, const void* buffer, std::size_t size, std::string& error_info)
{
    asn_codec_ctx_t ctx {};
    asn_dec_rval_t ec = uper_decode_complete(&ctx, &td, t, buffer, size);
    if (ec.code != RC_OK) {
        std::ostringstream oss;
        oss << "Decode failed for type '" << td.name << "': ";
        if (ec.code == RC_WMORE) {
            oss << "incomplete data (need more bytes)";
        } else {
            oss << "decode error";
        }
        oss << ", consumed " << ec.consumed << " of " << size << " bytes";
        error_info = oss.str();
        return false;
    }
    return true;
